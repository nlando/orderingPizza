...
let customerName = prompt("What is your name?", "John Doe");

let pieCount = prompt("How many pizzas will you be ordering? Enter integer e.g. (1, 3, etc.)", "2");
...
let pizzaSize = prompt("Enter the size of the pizza: S, M, or L", "S").toLowerCase();
let perPizzaPrice;
...
salesTax = (basePizzaPrice + extraToppingsFee + serviceFee) * taxRate;

totalPrice = basePizzaPrice + extraToppingsFee + salesTax + serviceFee;


alert(customerName + ", you ordered " + pieCount + " " + size + ", " + pizzaToppingsStatus + " pizza(s).");
alert("Base Price: $" + basePizzaPrice.toFixed(2));
alert("Service fee: $" + serviceFee.toFixed(2));
alert("Extra toppings fee: $" + extraToppingsFee.toFixed(2));
